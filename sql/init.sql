...
CREATE TABLE `app_info` (
  ...
  `release_type` tinyint(2) NOT NULL DEFAULT '1'  COMMENT '发布类型 1:zookeeper 2:http',
...
CREATE TABLE `env_info` (
  ...
  `create_time` datetime NOT NULL COMMENT '创建时间',
  `modify_time` datetime DEFAULT NULL COMMENT '修改时间',
  `status` char(2) NOT NULL COMMENT '状态 1开启 0关闭',
...
CREATE TABLE `system_config_info` (
  ...
  `status` tinyint(2) NOT NULL DEFAULT '0' COMMENT '发布状态 1已发布 0未发布',
...
CREATE TABLE `system_config_history` (
  ...
  `modifier` varchar(32) NOT NULL COMMENT '操作人',
...
CREATE TABLE `role_info` (
  `id` bigint(20) unsigned NOT NULL AUTO_INCREMENT COMMENT '自增ID',
  `name` varchar(30) NOT NULL COMMENT '角色标识',
  `description` varchar(30) NOT NULL COMMENT '角色描述',
  `create_time` datetime NOT NULL COMMENT '创建时间',
  `modify_time` datetime NOT NULL COMMENT '修改时间',
  `status` tinyint(2) NOT NULL COMMENT '权限状态',
  PRIMARY KEY (`id`)
